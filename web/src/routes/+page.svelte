<script lang="ts">
	...
	const { official, community } = data as {
		official: ViewInstance[];
		community: ViewInstance[];
		lastUpdatedUTC: string;
	};

	const fmtServices = (i: ViewInstance) => `${i.totals.up}/${i.totals.total}`;
	const safeHost = (h?: string) => h?.replace(/^https?:\/\//, '') ?? '—';

	// score -> rating class; offline wins
	const ratingClass = (i: ViewInstance) => {
...
<p>
	...
	This site gives an overall idea of what's working.
</p>
<p>
